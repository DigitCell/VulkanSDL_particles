...
//protected:	//TODO: make these both classes (structs are too open)
    ...
    virtual void RecreateVertexObject(VulkanSetup& vulkan) {};
};
//
...
public:
	...
	void Recreate(VulkanSetup& vulkan)
	{
		for (auto& recordable : recordables)
			for (auto& pRenderable : recordable.pRenderables)
                pRenderable->Recreate(vulkan, true);
	}

    void RecreateVertexBuffer(VulkanSetup& vulkan, size_t index)
    {
        auto& recordable= recordables[0];
        auto& pRenderable= recordable.pRenderables[index];
           pRenderable->RecreateVertexObject(vulkan);
    }
